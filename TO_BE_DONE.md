...
| 10 | pgvector hybrid retrieval | Pure-Python BM25 (`rag/retriever.py`) | Groq serves no embedding model, so this now needs a second provider (or a local sentence-transformers model) before it can be built. Add an `embed()` to the gateway, store vectors, and fuse cosine + BM25 by reciprocal rank. Keep `search_with_floor()`'s signature. |
